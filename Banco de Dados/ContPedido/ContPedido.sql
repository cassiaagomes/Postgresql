...
ALTER TABLE Setor 
	ALTER COLUMN idchefe SET NOT NULL;
	
/*** Em seguida, adicione a restrição UNIQUE ***/
ALTER TABLE Setor
	ADD CONSTRAINT UN_setor_idchefe UNIQUE (idchefe);

--------------- SELECIONANDO COLUNAS ---------------------
/*** Obter todos os dados de todos os clientes. ***/
SELECT * FROM Cliente;

/*** Obter todos os dados de todos os produtos. ***/
SELECT * FROM Produto;

/*** Obter código, nome e telefone de todos os clientes. ***/
SELECT idcliente, nome, fone
FROM Cliente;

/*** Obter código, data e valor do frete de todos os pedidos. ***/
SELECT idpedido, datapedid, frete
FROM Pedido;

---------------- ALTERANDO O TÍTULO DAS COLUNAS -------------------
/*** Obter o código com o título “Produto” e o preço de venda com
o título “Preço de venda” de todos os produtos. ***/
SELECT idproduto AS "Produto",
	venda AS "Preço de venda"
FROM Produto;

/*** Obter o nome com o título “Função” e o valor da gratificação
com o título “Gratificação” de todas as funções. ***/
SELECT nome AS "Função", 
	gratific AS "Gratificação"
FROM Funcao;

---------------- ELIMINANDO DUPLICAÇÃO DE LINHAS -------------------
/*** Obter o código de todos os clientes que já fizeram pedido na
empresa (sem repetições). ***/
SELECT DISTINCT idcliente
FROM Pedido;

/*** Obter o bairro e o código da cidade de todos os funcionários
da empresa (sem repetições). ***/
SELECT DISTINCT bairro, idreside
FROM Funcionario;

---------------- EXIBINDO COLUNAS CALCULADAS -----------------------
/*** Obter código, quantidade em estoque, preço de venda e
(quantidade x preço de venda) para cada produto. ***/
SELECT idproduto, quantest, venda,
	quantest * venda AS "Valor"
FROM Produto;

/*** Obter código, nome, salário e o salário com 32% de aumento
para todos os funcionários. ***/
SELECT idfuncionario, nome, salario,
	salario * 1.32 AS "Sal.c/Aumento"
FROM Funcionario;

------------------- ORDENANDO RESULTADOS ----------------------------
/*** Obter código, nome, contato e cargo de todos os clientes,
ordenados pelo cargo em ordem ascendente. ***/
SELECT idcliente, nome, contato, cargo
FROM Cliente
ORDER BY cargo;

/*** Obter código, nome, estado civil e data de nascimento de
...
/*** Obter código, nome, preço de custo, preço de venda e a
diferença entre esses dois preços para todos os produtos, ordenados pela
diferença em ordem descendente. ***/
SELECT idproduto, nome, custo, venda,
	venda - custo AS "Diferença"
FROM Produto
ORDER BY venda - custo DESC;

/*** Obter código, nome e tipo de todos os produtos, ordenados
pelo tipo em ordem ascendente e pelo nome em ordem descendente. ***/
SELECT idproduto, nome, idtipo
FROM Produto
ORDER BY idtipo ASC, nome DESC;

/*** Obter código, nome, cidade onde reside, sexo e salário de
todos os funcionários, ordenados por cidade e sexo em ordem ascendente
e por salário em ordem descendente. ***/
...
-------------------- FILTRAGEM DE LINHAS ---------------------------
/*** Obter código, nome, tipo, quantidade em estoque e preço de
venda dos produtos que tenham quantidade em estoque maior que 20 ou
preço de venda inferior a R$ 50,00, ordenados pelo preço de venda em
ordem descendente. ***/
...
/*** Obter todos os dados dos clientes que são do tipo pessoa
jurídica e que não sejam da cidade de código igual a 14, ou que são do tipo
pessoa física e da cidade de código igual a 14, ordenando pelo nome em
ordem ascendente. ***/
SELECT *
FROM Cliente
WHERE (tipo = 'J' and idcidade <> 14) or
	  (tipo = 'F' and idcidade = 14)
ORDER BY nome;

/*** Obter código, nome, endereço e bairro dos funcionários que
residam em Manaíra, Tambaú ou Bessa, ordenados pelo nome do
funcionário em ordem ascendente. ***/
SELECT idfuncionario, nome, endereco, bairro
FROM Funcionario
WHERE bairro IN ('Manaíra','Tambaú','Bessa')
ORDER BY nome;

/*** Obter código, data e via de transporte de todos os pedidos
que tenham sido transportados por via aérea ou marítima. ***/
SELECT idpedido, datapedid, via
FROM Pedido
WHERE via IN ('A','M');

/*** Obter código, nome e tipo de todos os produtos que não
sejam do tipo 2 ou 4, ordenados pelo tipo em ordem descendente e pelo
...
/*** Obter todos os dados dos produtos com quantidade em
estoque entre 10 e 30 unidades, ordenados pelo nome do produto em
ordem ascendente. ***/
SELECT *
FROM Produto
...
/*** Obter o nome e a data de nascimento dos funcionários que
nasceram na década de 60, ordenados por data de nascimento em ordem
descendente e por nome em ordem ascendente. ***/
SELECT nome, datanasc
FROM Funcionario
WHERE datanasc BETWEEN '1960/01/01' and '1969/12/31'
ORDER BY datanasc DESC, nome;

/*** Obter código, nome e email dos funcionários que não tenham
email, ordenando pelo nome em ordem ascendente. ***/
SELECT idfuncionario, nome, email
FROM Funcionario
WHERE email IS NULL
ORDER BY nome;

/*** Obter tipo, código, nome e fax dos clientes que tenham fax,
ordenados pelo tipo e pelo nome. ***/
SELECT tipo, idcliente, nome, fax
FROM Cliente
WHERE fax IS NOT NULL
ORDER BY tipo, nome;

/*** Obter código, nome, email e celular dos funcionários que não
tenham email mas possuam telefone celular, ordenados pelo nome em
ordem ascendente. ***/
...
/*** Obter o código e o nome de todos os funcionários que
